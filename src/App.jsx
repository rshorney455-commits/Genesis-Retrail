  ...
  useEffect(() => {
    ...
    function initMap() {
      ...
      const siteIcon = L.divIcon({
        html: `<div style="background:#2d6a4f;color:#fff;border-radius:50%;width:32px;height:32px;display:flex;align-items:center;justify-content:center;font-size:16px;border:3px solid #fff;box-shadow:0 2px 8px rgba(0,0,0,0.3)">🏪</div>`,
      ...
      (competitors || []).forEach((c, i) => {
        const col = c.threat === "high" ? "#d62828" : c.threat === "medium" ? "#e07020" : "#40916c";
      ...
      L.circle([lat, lng], { radius: 804, color: "#2d6a4f", fillColor:"#2d6a4f", fillOpacity:0.05, weight:1.5, dashArray:"6,4" }).addTo(map);
    }
...
function RiskRegister({ risks }) {
  const cols = { red:"#d62828", amber:"#e07020", green:"#2d6a4f" };
  return (
    <div>
      {risks.map((r, i) => (
    ...
    <div>
      {scores.map((sg, i) => (
        <div key={sg.name} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",marginBottom:8,background:i===0?G.pale:G.card,border:"1px solid "+(i===0?G.mid:G.border),borderRadius:10}}>
          ...
          <div style={{flex:1}}>
            <div style={{height:6,background:"#e0ede6",borderRadius:3}}>
...
{"status":"ready","site":"${(payload.propName||"").replace(/"/g,"'")}","roi":${payload.derived.roi.toFixed(1)},"verdict":"${payload.verdict}"}`
  ...
  function buildHTMLPresentation(d, fmt2, pct2) {
    const GREEN = "#2D6A4F", DARK = "#1A3D2B", PALE = "#D8EDE3", ORANGE = "#E07020";
    const slides = [
      // Cover
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; font-family: Calibri, Arial, sans-serif; }
  ...
  .slide-header { background: #1A3D2B; padding: 14px 20px 12px; display:flex; justify-content:space-between; align-items:flex-end; }
  .slide-header h3 { color: #fff; font-size: 20px; font-weight: 700; }
  .slide-sub, .site-tag { color: #A8D5B5; font-size: 10px; }
  .cover { flex-direction: row; min-height: 180mm; }
  .cover-left { background: #2D6A4F; padding: 28px 24px; width: 55%; display:flex; flex-direction:column; }
  .cover-right { background: #1A3D2B; padding: 28px 20px; flex: 1; }
  .brand { color: #A8D5B5; font-size: 9px; letter-spacing: 4px; font-weight: 700; margin-bottom: 8px; }
  .cover-left h1 { color: #fff; font-size: 30px; font-weight: 800; line-height: 1.15; margin-bottom: 14px; }
  .cover-left h2 { color: #D8EDE3; font-size: 16px; font-weight: 700; margin-bottom: 6px; }
  .cover-left p.sub { color: #A8D5B5; font-size: 11px; margin-bottom: 18px; }
  .badge-row { display:flex; gap: 10px; margin-bottom: 16px; }
  .badge { background: #1B4332; border: 1px solid #40916C; padding: 8px 12px; border-radius: 4px; flex: 1; }
  .badge-lbl { color: #A8D5B5; font-size: 8px; font-weight: 700; letter-spacing: 2px; margin-bottom: 3px; }
  .badge-val { color: #fff; font-size: 15px; font-weight: 800; }
  .prepared { color: #6B9E83; font-size: 9px; margin-top: auto; }
  .kpi-title { color: #A8D5B5; font-size: 9px; letter-spacing: 2px; font-weight: 700; margin-bottom: 12px; }
  .kpi-row { background: #1B4332; border: 1px solid #40916C; padding: 7px 10px; margin-bottom: 5px; display:flex; justify-content:space-between; align-items:center; }
  .kpi-lbl { color: #A8D5B5; font-size: 9px; }
  .kpi-val { color: #fff; font-size: 15px; font-weight: 800; }
  .verdict-bar { background: #2D6A4F; color: #fff; text-align:center; padding: 10px; font-size: 12px; font-weight: 700; }
  .stat-grid { display:grid; grid-template-columns:1fr 1fr 1fr 1fr; gap: 10px; padding: 14px 16px; }
  .stat-card { border: 1px solid #D8EDE3; padding: 12px; background: #F4F9F6; }
  .stat-lbl { font-size: 8px; color: #6B9E83; letter-spacing: 1px; font-weight: 700; margin-bottom: 5px; }
  .stat-val { font-size: 20px; font-weight: 800; color: #2D6A4F; margin-bottom: 4px; }
  .stat-sub { font-size: 9px; color: #6B9E83; }
  ...
  .kpi9 { border: 1px solid #D8EDE3; padding: 14px; background: #F4F9F6; }
  .kpi9-lbl { font-size: 8px; color: #6B9E83; letter-spacing: 1px; font-weight: 700; margin-bottom: 6px; }
  .kpi9-val { font-size: 22px; font-weight: 800; color: #2D6A4F; margin-bottom: 4px; }
  .kpi9-sub { font-size: 9px; color: #6B9E83; }
  .two-col { display:flex; gap: 16px; padding: 14px 16px; flex:1; }
  .two-col > * { flex: 1; }
  table { border-collapse: collapse; width: 100%; font-size: 10.5px; }
  th { background: #2D6A4F; color: #fff; padding: 7px 10px; text-align: left; font-size: 10px; }
  td { padding: 7px 10px; border-bottom: 1px solid #D8EDE3; color: #334155; }
  .highlight-row td { background: #D8EDE3; font-weight: 700; color: #2D6A4F; }
  .income-row td { background: #F4F9F6; font-weight: 700; }
  .ebitda-row td { background: #D8F0E0; font-weight: 700; color: #2D6A4F; }
  .net-row td { background: #B7E4C7; font-weight: 800; color: #1A3D2B; font-size: 12px; }
  .neg { color: #C05010 !important; }
  .pos { color: #2D6A4F !important; }
  ...
  .bar-track { flex:1; height: 22px; background: #D8EDE3; border-radius: 3px; overflow:hidden; }
  .bar-fill { height: 100%; background: #2D6A4F; border-radius: 3px; }
  .bar-value { font-size: 11px; font-weight: 700; color: #2D6A4F; width: 60px; text-align:right; flex-shrink:0; }
  .full-table { font-size: 9.5px; }
  .progress-row { display:flex; gap: 12px; padding: 12px 16px 8px; align-items:flex-end; border-top: 1px solid #D8EDE3; margin-top: 8px; }
  .prog-col { flex:1; display:flex; flex-direction:column; align-items:center; gap:4px; }
  .prog-lbl { font-size: 9px; color: #6B9E83; font-weight: 700; }
  .prog-bar { width:100%; height:50px; background:#F4F9F6; border-radius:3px; display:flex; align-items:flex-end; border:1px solid #D8EDE3; }
  .prog-fill { width:100%; background:#2D6A4F; border-radius:3px; min-height:3px; }
  .prog-val { font-size: 9px; color: #2D6A4F; font-weight: 700; }
  .section-title { font-size: 9px; font-weight: 700; color: #2D6A4F; letter-spacing: 2px; margin-bottom: 10px; }
  .simple-table { font-size: 10.5px; margin-bottom: 12px; }
  .big-roi { font-size: 50px; font-weight: 800; color: #2D6A4F; margin: 10px 0 4px; }
  .roi-lbl { font-size: 9px; letter-spacing: 2px; color: #6B9E83; font-weight: 700; margin-bottom: 4px; }
  .roi-verdict { font-size: 13px; font-weight: 700; color: #2D6A4F; margin-bottom: 10px; }
  .payback-box { background: #1A3D2B; color: #fff; padding: 8px 14px; font-size: 14px; font-weight: 700; display:inline-block; margin-bottom: 10px; }
  .threshold-list { font-size: 9.5px; margin-bottom: 10px; }
  .t-row { padding: 3px 0; font-weight: 700; }
  .t-row.green { color: #2D6A4F; }
  .t-row.amber { color: #E07020; }
  .t-row.red { color: #D62828; }
  .lease-box { background: #F4F9F6; border: 1px solid #D8EDE3; padding: 10px; }
  .lease-title { font-size: 8px; color: #6B9E83; letter-spacing: 1.5px; font-weight: 700; margin-bottom: 4px; }
  .lease-val { font-size: 20px; font-weight: 800; color: #2D6A4F; }
  .lease-note { font-size: 9px; color: #6B9E83; }
  ...
  .risk-row.green { background: #E8F5E0; border-color: #2D6A4F; }
  ...
  .risk-row.green .risk-bar { background: #2D6A4F; }
  .risk-row.amber .risk-bar { background: #E07020; }
  .risk-row.red .risk-bar { background: #D62828; }
  .risk-body { flex:1; padding: 8px 12px; }
  .risk-title { font-size: 11px; font-weight: 700; color: #1A3D2B; margin-bottom: 3px; }
  ...
  .risk-row.green .risk-badge { color: #2D6A4F; }
  .risk-row.amber .risk-badge { color: #E07020; }
  .risk-row.red .risk-badge { color: #D62828; }
  .risk-legend { padding: 6px 16px; font-size: 9.5px; color: #6B9E83; display:flex; gap:16px; border-top:1px solid #D8EDE3; }
  ...
  .verdict-label { color: #A8D5B5; font-size: 9px; letter-spacing: 3px; font-weight: 700; margin-bottom: 6px; }
  .conc-stats { display:grid; grid-template-columns:1fr 1fr; gap:8px; margin-top: 16px; }
  .cs-lbl { color: #A8D5B5; font-size: 8px; letter-spacing: 1.5px; font-weight: 700; margin-bottom: 3px; }
  .cs-val { color: #fff; font-size: 17px; font-weight: 800; }
  .rec-text { font-size: 11px; color: #D8EDE3; line-height: 1.7; margin-bottom: 14px; }
  .rec-text strong { color: #fff; }
  .disclaimer { font-size: 8.5px; color: #6B9E83; font-style: italic; margin-top: auto; }
...
${slides.join("\n")}
  ...
  return (
    <button
      ...
      style={{width:"100%",padding:12,background:status==="generating"?"#d8ede3":G.orange,border:"none",borderRadius:8,color:status==="generating"?G.mid:"#fff",cursor:status==="generating"?"default":"pointer",fontFamily:"inherit",fontSize:14,fontWeight:700}}
    >
      ...
      <div style={{padding:"24px 16px 80px",maxWidth:700,margin:"0 auto"}}>
        ...
        {step===1&&(
          <div>
            ...
            {/* Postcode lookup */}
            <div style={{background:"#e8f5e0",border:"1.5px solid "+G.mid,borderRadius:12,padding:16,marginBottom:20}}>
        ...
        {step===2&&(
          <div>
            ...
            <Legend/>
            {postcodeData&&<div style={{background:"#e8f5e0",border:"1px solid "+G.border,borderRadius:8,padding:"10px 14px",marginBottom:16,fontSize:13,color:G.mid}}>✓ Business rates auto-estimated from VOA data for {postcode}. Override if you have the actual figure.</div>}
        ...
        {step===5&&(
          <div>
            ...
            <Legend/>
            {postcodeData&&<div style={{background:"#e8f5e0",border:"1px solid "+G.border,borderRadius:8,padding:"10px 14px",marginBottom:16,fontSize:13,color:G.mid}}>✓ Regional income and population estimates auto-filled from postcode data. Override with ONS census figures for greater accuracy.</div>}
            <Row2 ch={[
        ...
        {step===8&&(
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:20}}>
              {/* Excel export */}
              <div style={{background:"#e8f5e0",border:"1.5px solid "+G.mid,borderRadius:12,padding:16}}>
            ...
            {(()=>{
              ...
              const TD = (v,bold,neg,hi,pctFmt)=>(
                <td style={{padding:"7px 10px",fontSize:12,textAlign:"right",fontWeight:bold?700:400,color:neg?"#d62828":hi?G.mid:G.dark,background:hi?"#e8f5e0":"transparent",borderBottom:"1px solid "+G.border,whiteSpace:"nowrap"}}>
                    ...
                    {sheet==="pl"&&(
                      <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                        ...
                        <tbody>
                          ...
                          <tr style={{background:"#c6ebd4"}}>{TDL("NET PROFIT",true,false,true)}{yr5x.map((r,i)=><td key={i} style={{padding:"7px 10px",textAlign:"right",fontSize:13,fontWeight:800,color:r.np>=0?G.mid:"#d62828",borderBottom:"1px solid "+G.border}}>{r.np<0?"("+fmt(Math.abs(r.np))+")":fmt(r.np)}</td>)}<td style={{padding:"7px 10px",textAlign:"right",fontSize:13,fontWeight:800,color:yr5x.reduce((a,r)=>a+r.np,0)>=0?G.mid:"#d62828",borderBottom:"1px solid "+G.border}}>{fmt(yr5x.reduce((a,r)=>a+r.np,0))}</td></tr>
                          <tr style={{background:"#b7e4c7"}}>{TDL("CUMULATIVE NET PROFIT",true,false,true)}{yr5x.map((r,i)=><td key={i} style={{padding:"7px 10px",textAlign:"right",fontSize:12,fontWeight:700,color:r.cum>=0?G.mid:"#d62828",borderBottom:"1px solid "+G.border}}>{r.cum<0?"("+fmt(Math.abs(r.cum))+")":fmt(r.cum)}</td>)}<td style={{padding:"7px 10px",textAlign:"right",fontSize:12,color:G.light,borderBottom:"1px solid "+G.border}}>—</td></tr>
                        ...
                        <tbody>
                          {(()=>{
                            ...
                            return cfItems.map((item,idx)=>{
                              ...
                              const bg = item.hi?"#e8f5e0":item.hi2?"#c6ebd4":"transparent";
                            ...
                            {sensitivityData.map((row,ri)=>(
                              <tr key={ri}>
                                ...
                                {row.map((cell,ci)=>{
                                  ...
                                  const bg=isBase?"#d8ede3":cell.roi>=20?"#e8f5e0":cell.roi>=10?"#fff4ea":"#fde8e8";
            ...
            <div style={{minHeight:"90vh",display:"flex",flexDirection:"column",borderBottom:"3px solid "+G.mid,marginBottom:28,paddingBottom:32}}>
              <div style={{background:G.mid,borderRadius:10,padding:"18px 20px",marginBottom:20,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12}}>
                <div>
                  ...
                  <div style={{fontSize:12,color:"#a8d5b5",marginTop:3}}>{postcode&&postcode+" · "}Genesis Retail · {new Date().toLocaleDateString("en-GB",{day:"numeric",month:"long",year:"numeric"})}</div>
                </div>
                    ...
                    {planningApps.map((pa,i)=>(
                      <div key={i} style={{padding:"10px 0",borderBottom:i<planningApps.length-1?"1px solid "+G.border:"none"}}>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8}}>
                          ...
                          <div style={{padding:"3px 10px",borderRadius:4,fontSize:11,fontWeight:700,background:pa.risk==="high"?"#fde8e8":pa.risk==="medium"?"#fff4ea":"#e8f5e0",color:pa.risk==="high"?"#d62828":pa.risk==="medium"?G.orange:G.mid,flexShrink:0}}>{pa.risk.toUpperCase()}</div>
                        </div>
                      </div>
                    ))}
              ...
              <div className="page-break avoid-break">
                ...
                <RC t="Benchmark Comparison" ch={
                  <div>
                    ...
                    <div style={{display:"flex",padding:"10px 0",borderBottom:"1px solid "+G.border,background:"#e8f5e0"}}>
