...
const TCOLORS     = {"Rising Rapidly":"#0d2b8a","Rising Steadily":"#1a3fbf","Stable":"#d4a017","Declining Slightly":"#c05010","Declining Rapidly":"#d62828"};
...
const G = {
  bg:"#f5f6fa", card:"#edf0f9", border:"#c5cce8",
  text:"#0d1b3e", dark:"#060e24", mid:"#0d2b8a", light:"#1a3fbf", pale:"#d5dcf5",
  orange:"#d4a017", obg:"#fdf6e3",
};

const INP_manual = {width:"100%",padding:"12px 14px",background:"#fffbee",border:"2px solid #d4a017",borderRadius:8,color:"#7a5c00",fontFamily:"inherit",fontSize:16,outline:"none",WebkitAppearance:"none",appearance:"none",fontWeight:700};
const INP_auto   = {width:"100%",padding:"12px 14px",background:"#e8ecf9",border:"2px solid #0d2b8a",borderRadius:8,color:"#0d2b8a",fontFamily:"inherit",fontSize:16,outline:"none",WebkitAppearance:"none",appearance:"none",fontWeight:700};
  ...
  useEffect(() => {
    ...
    function initMap() {
      ...
      const siteIcon = L.divIcon({
        html: `<div style="background:#0d2b8a;color:#fff;border-radius:50%;width:32px;height:32px;display:flex;align-items:center;justify-content:center;font-size:16px;border:3px solid #fff;box-shadow:0 2px 8px rgba(0,0,0,0.3)">🏪</div>`,
      ...
      (competitors || []).forEach((c, i) => {
        const col = c.threat === "high" ? "#d62828" : c.threat === "medium" ? "#e07020" : "#1a3fbf";
      ...
      L.circle([lat, lng], { radius: 804, color: "#0d2b8a", fillColor:"#0d2b8a", fillOpacity:0.05, weight:1.5, dashArray:"6,4" }).addTo(map);
    }
...
function RiskRegister({ risks }) {
  const cols = { red:"#d62828", amber:"#e07020", green:"#0d2b8a" };
  return (
    <div>
      {risks.map((r, i) => (
    ...
    <div>
      {scores.map((sg, i) => (
        <div key={sg.name} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",marginBottom:8,background:i===0?G.pale:G.card,border:"1px solid "+(i===0?G.mid:G.border),borderRadius:10}}>
          ...
          <div style={{flex:1}}>
            <div style={{height:6,background:"#e8ecf9",borderRadius:3}}>
...
function Donut({data}){
  ...
  const COLS=["#0d2b8a","#1a3fbf","#1a3fbf","#4a6fd4","#7a9ae0","#c5d0f0","#060e24","#060e24","#e07020","#f4a04a","#ffd166","#1a3fbf","#118ab2"];
  let cum=0;
...
{"status":"ready","site":"${(payload.propName||"").replace(/"/g,"'")}","roi":${payload.derived.roi.toFixed(1)},"verdict":"${payload.verdict}"}`
  ...
  function buildHTMLPresentation(d, fmt2, pct2) {
    const GREEN = "#0d2b8a", DARK = "#060e24", PALE = "#d5dcf5", ORANGE = "#E07020";
    const slides = [
      // Cover
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; font-family: Calibri, Arial, sans-serif; }
  ...
  .slide-header { background: #060e24; padding: 14px 20px 12px; display:flex; justify-content:space-between; align-items:flex-end; }
  ...
  .cover-left { background: #0d2b8a; padding: 28px 24px; width: 55%; display:flex; flex-direction:column; }
  .cover-right { background: #060e24; padding: 28px 20px; flex: 1; }
  ...
  .cover-left h2 { color: #d5dcf5; font-size: 16px; font-weight: 700; margin-bottom: 6px; }
  .cover-left p.sub { color: #8fa8d8; font-size: 11px; margin-bottom: 18px; }
  .badge-row { display:flex; gap: 10px; margin-bottom: 16px; }
  .badge { background: #060e24; border: 1px solid #1a3fbf; padding: 8px 12px; border-radius: 4px; flex: 1; }
  ...
  .kpi-row { background: #060e24; border: 1px solid #1a3fbf; padding: 7px 10px; margin-bottom: 5px; display:flex; justify-content:space-between; align-items:center; }
  .kpi-lbl { color: #8fa8d8; font-size: 9px; }
  .kpi-val { color: #fff; font-size: 15px; font-weight: 800; }
  .verdict-bar { background: #0d2b8a; color: #fff; text-align:center; padding: 10px; font-size: 12px; font-weight: 700; }
  .stat-grid { display:grid; grid-template-columns:1fr 1fr 1fr 1fr; gap: 10px; padding: 14px 16px; }
  .stat-card { border: 1px solid #d5dcf5; padding: 12px; background: #F4F9F6; }
  .stat-lbl { font-size: 8px; color: #5a6fa8; letter-spacing: 1px; font-weight: 700; margin-bottom: 5px; }
  .stat-val { font-size: 20px; font-weight: 800; color: #0d2b8a; margin-bottom: 4px; }
  ...
  .kpi9 { border: 1px solid #d5dcf5; padding: 14px; background: #F4F9F6; }
  .kpi9-lbl { font-size: 8px; color: #5a6fa8; letter-spacing: 1px; font-weight: 700; margin-bottom: 6px; }
  .kpi9-val { font-size: 22px; font-weight: 800; color: #0d2b8a; margin-bottom: 4px; }
  .kpi9-sub { font-size: 9px; color: #5a6fa8; }
  .two-col { display:flex; gap: 16px; padding: 14px 16px; flex:1; }
  .two-col > * { flex: 1; }
  table { border-collapse: collapse; width: 100%; font-size: 10.5px; }
  th { background: #0d2b8a; color: #fff; padding: 7px 10px; text-align: left; font-size: 10px; }
  td { padding: 7px 10px; border-bottom: 1px solid #d5dcf5; color: #334155; }
  .highlight-row td { background: #d5dcf5; font-weight: 700; color: #0d2b8a; }
  .income-row td { background: #F4F9F6; font-weight: 700; }
  .ebitda-row td { background: #dce3f8; font-weight: 700; color: #0d2b8a; }
  .net-row td { background: #c5d0f0; font-weight: 800; color: #060e24; font-size: 12px; }
  .neg { color: #C05010 !important; }
  .pos { color: #0d2b8a !important; }
  ...
  .bar-track { flex:1; height: 22px; background: #d5dcf5; border-radius: 3px; overflow:hidden; }
  .bar-fill { height: 100%; background: #0d2b8a; border-radius: 3px; }
  .bar-value { font-size: 11px; font-weight: 700; color: #0d2b8a; width: 60px; text-align:right; flex-shrink:0; }
  .full-table { font-size: 9.5px; }
  .progress-row { display:flex; gap: 12px; padding: 12px 16px 8px; align-items:flex-end; border-top: 1px solid #d5dcf5; margin-top: 8px; }
  ...
  .prog-bar { width:100%; height:50px; background:#F4F9F6; border-radius:3px; display:flex; align-items:flex-end; border:1px solid #d5dcf5; }
  .prog-fill { width:100%; background:#0d2b8a; border-radius:3px; min-height:3px; }
  .prog-val { font-size: 9px; color: #0d2b8a; font-weight: 700; }
  .section-title { font-size: 9px; font-weight: 700; color: #0d2b8a; letter-spacing: 2px; margin-bottom: 10px; }
  .simple-table { font-size: 10.5px; margin-bottom: 12px; }
  .big-roi { font-size: 50px; font-weight: 800; color: #0d2b8a; margin: 10px 0 4px; }
  .roi-lbl { font-size: 9px; letter-spacing: 2px; color: #5a6fa8; font-weight: 700; margin-bottom: 4px; }
  .roi-verdict { font-size: 13px; font-weight: 700; color: #0d2b8a; margin-bottom: 10px; }
  .payback-box { background: #060e24; color: #fff; padding: 8px 14px; font-size: 14px; font-weight: 700; display:inline-block; margin-bottom: 10px; }
  .threshold-list { font-size: 9.5px; margin-bottom: 10px; }
  .t-row { padding: 3px 0; font-weight: 700; }
  .t-row.green { color: #0d2b8a; }
  .t-row.amber { color: #E07020; }
  .t-row.red { color: #D62828; }
  .lease-box { background: #F4F9F6; border: 1px solid #d5dcf5; padding: 10px; }
  .lease-title { font-size: 8px; color: #5a6fa8; letter-spacing: 1.5px; font-weight: 700; margin-bottom: 4px; }
  .lease-val { font-size: 20px; font-weight: 800; color: #0d2b8a; }
  ...
  .risk-row.green { background: #e8ecf9; border-color: #0d2b8a; }
  ...
  .risk-row.green .risk-bar { background: #0d2b8a; }
  .risk-row.amber .risk-bar { background: #E07020; }
  .risk-row.red .risk-bar { background: #D62828; }
  .risk-body { flex:1; padding: 8px 12px; }
  .risk-title { font-size: 11px; font-weight: 700; color: #060e24; margin-bottom: 3px; }
  ...
  .risk-row.green .risk-badge { color: #0d2b8a; }
  .risk-row.amber .risk-badge { color: #E07020; }
  .risk-row.red .risk-badge { color: #D62828; }
  .risk-legend { padding: 6px 16px; font-size: 9.5px; color: #5a6fa8; display:flex; gap:16px; border-top:1px solid #d5dcf5; }
  .dot { display:inline-block; width:10px; height:10px; border-radius:50%; }
  .dot.green { background:#0d2b8a; }
  ...
  .rec-text { font-size: 11px; color: #d5dcf5; line-height: 1.7; margin-bottom: 14px; }
...
${slides.join("\n")}
  ...
  return (
    <button
      ...
      style={{width:"100%",padding:12,background:status==="generating"?"#d5dcf5":G.orange,border:"none",borderRadius:8,color:status==="generating"?G.mid:"#fff",cursor:status==="generating"?"default":"pointer",fontFamily:"inherit",fontSize:14,fontWeight:700}}
    >
    ...
    <div style={{fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif",background:G.bg,minHeight:"100vh",color:G.text}}>
      <style>{`
        *{box-sizing:border-box;margin:0}
        ...
        select option{background:#fff;color:#060e24}
        textarea{resize:vertical}
        ::-webkit-scrollbar{width:5px}::-webkit-scrollbar-thumb{background:#c5cce8;border-radius:3px}
      ...
      {/* Header nav */}
      <div className="no-print" style={{background:"#060e24",padding:"16px 16px 0",position:"sticky",top:0,zIndex:100,boxShadow:"0 4px 20px rgba(6,14,36,0.6)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
          <div>
            <div style={{fontSize:11,letterSpacing:".18em",color:"#d4a017",textTransform:"uppercase",marginBottom:3}}>Convenience Retail</div>
          ...
          <div style={{display:"flex",gap:6,flexShrink:0,marginTop:4}}>
            <button onClick={saveAssessment} style={{padding:"7px 12px",background:"rgba(212,160,23,0.15)",border:"1.5px solid #d4a017",borderRadius:7,color:"#d4a017",cursor:"pointer",fontFamily:"inherit",fontSize:12,fontWeight:700}}>
              {saveMsg||"💾 Save"}
            </button>
            <button onClick={()=>setShowShare(true)} style={{padding:"7px 12px",background:"rgba(212,160,23,0.15)",border:"1.5px solid #d4a017",borderRadius:7,color:"#d4a017",cursor:"pointer",fontFamily:"inherit",fontSize:12,fontWeight:700}}>
              🔒 Share
            </button>
          </div>
        </div>
        <div style={{display:"flex",gap:2,overflowX:"auto",WebkitOverflowScrolling:"touch"}}>
          {STEPS.map((s,i)=>(
            <button key={i} onClick={()=>setStep(i)} style={{flexShrink:0,padding:"8px 12px",background:step===i?"#fff":step>i?"#d4a017":"transparent",border:"1.5px solid "+(step===i?"#fff":step>i?"#d4a017":"#5a6fa8"),color:step===i?G.mid:step>i?"#060e24":"#8fa8d8",fontSize:12,borderRadius:"6px 6px 0 0",whiteSpace:"nowrap",cursor:"pointer",fontFamily:"inherit",fontWeight:step===i?700:400}}>
              {i+1}. {s}
            </button>
          ))}
        </div>
      ...
      <div style={{padding:"24px 16px 80px",maxWidth:700,margin:"0 auto"}}>
        ...
        {step===1&&(
          <div>
            ...
            {/* Postcode lookup */}
            <div style={{background:"#e8ecf9",border:"1.5px solid "+G.mid,borderRadius:12,padding:16,marginBottom:20}}>
        ...
        {step===2&&(
          <div>
            ...
            <Legend/>
            {postcodeData&&<div style={{background:"#e8ecf9",border:"1px solid "+G.border,borderRadius:8,padding:"10px 14px",marginBottom:16,fontSize:13,color:G.mid}}>✓ Business rates auto-estimated from VOA data for {postcode}. Override if you have the actual figure.</div>}
        ...
        {step===4&&(
          <div>
            ...
            <Legend/>
            <div style={{padding:"12px 14px",borderRadius:8,marginBottom:16,background:totalMix===100?"#e8ecf9":"#fff4ea",border:"1px solid "+(totalMix===100?"#1a3fbf":G.orange),fontSize:14,color:totalMix===100?G.mid:G.orange,fontWeight:600}}>
        ...
        {step===5&&(
          <div>
            ...
            <Legend/>
            {postcodeData&&<div style={{background:"#e8ecf9",border:"1px solid "+G.border,borderRadius:8,padding:"10px 14px",marginBottom:16,fontSize:13,color:G.mid}}>✓ Regional income and population estimates auto-filled from postcode data. Override with ONS census figures for greater accuracy.</div>}
            <Row2 ch={[
        ...
        {step===8&&(
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:20}}>
              {/* Excel export */}
              <div style={{background:"#e8ecf9",border:"1.5px solid "+G.mid,borderRadius:12,padding:16}}>
            ...
            {(()=>{
              ...
              const TD = (v,bold,neg,hi,pctFmt)=>(
                <td style={{padding:"7px 10px",fontSize:12,textAlign:"right",fontWeight:bold?700:400,color:neg?"#d62828":hi?G.mid:G.dark,background:hi?"#e8ecf9":"transparent",borderBottom:"1px solid "+G.border,whiteSpace:"nowrap"}}>
                    ...
                    {sheet==="pl"&&(
                      <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                        ...
                        <tbody>
                          ...
                          <tr style={{background:"#c5d0f0"}}>{TDL("NET PROFIT",true,false,true)}{yr5x.map((r,i)=><td key={i} style={{padding:"7px 10px",textAlign:"right",fontSize:13,fontWeight:800,color:r.np>=0?G.mid:"#d62828",borderBottom:"1px solid "+G.border}}>{r.np<0?"("+fmt(Math.abs(r.np))+")":fmt(r.np)}</td>)}<td style={{padding:"7px 10px",textAlign:"right",fontSize:13,fontWeight:800,color:yr5x.reduce((a,r)=>a+r.np,0)>=0?G.mid:"#d62828",borderBottom:"1px solid "+G.border}}>{fmt(yr5x.reduce((a,r)=>a+r.np,0))}</td></tr>
                          <tr style={{background:"#c5d0f0"}}>{TDL("CUMULATIVE NET PROFIT",true,false,true)}{yr5x.map((r,i)=><td key={i} style={{padding:"7px 10px",textAlign:"right",fontSize:12,fontWeight:700,color:r.cum>=0?G.mid:"#d62828",borderBottom:"1px solid "+G.border}}>{r.cum<0?"("+fmt(Math.abs(r.cum))+")":fmt(r.cum)}</td>)}<td style={{padding:"7px 10px",textAlign:"right",fontSize:12,color:G.light,borderBottom:"1px solid "+G.border}}>—</td></tr>
                        ...
                        <tbody>
                          {(()=>{
                            ...
                            return cfItems.map((item,idx)=>{
                              ...
                              const bg = item.hi?"#e8ecf9":item.hi2?"#c5d0f0":"transparent";
                            ...
                            {sensitivityData.map((row,ri)=>(
                              <tr key={ri}>
                                ...
                                {row.map((cell,ci)=>{
                                  ...
                                  const bg=isBase?"#d5dcf5":cell.roi>=20?"#e8ecf9":cell.roi>=10?"#fff4ea":"#fde8e8";
        ...
        {step===9&&(
          <div>
                    ...
                    {planningApps.map((pa,i)=>(
                      <div key={i} style={{padding:"10px 0",borderBottom:i<planningApps.length-1?"1px solid "+G.border:"none"}}>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8}}>
                          ...
                          <div style={{padding:"3px 10px",borderRadius:4,fontSize:11,fontWeight:700,background:pa.risk==="high"?"#fde8e8":pa.risk==="medium"?"#fff4ea":"#e8ecf9",color:pa.risk==="high"?"#d62828":pa.risk==="medium"?G.orange:G.mid,flexShrink:0}}>{pa.risk.toUpperCase()}</div>
                        </div>
                      </div>
                    ))}
            ...
            <div className="page-break avoid-break">
              ...
              <div style={{background:G.card,border:"1px solid "+G.border,borderRadius:12,overflow:"hidden",marginBottom:20}}>
                {[
                  ...
                  return <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:(r.type==="sub"?"10px":"7px")+" 16px",borderBottom:"1px solid "+G.border,background:r.hi?(r.v>=0?"#e8ecf9":"#fde8e8"):r.type==="sub"?G.pale:"transparent"}}>
                ...
                <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                  ...
                  <tbody>
                    {[
                      ...
                      <tr key={i} style={{background:dr.hi?"#e8ecf9":dr.sub?G.pale:i%2===0?G.card:"#fff",borderBottom:"1px solid "+G.border}}>
            ...
            <div className="page-break avoid-break">
              ...
              <div style={{background:"#e8ecf9",border:"1px solid "+G.border,borderRadius:8,padding:"10px 14px",marginBottom:16,fontSize:13,color:G.mid}}>
              ...
              <div className="page-break avoid-break">
                ...
                <RC t="Benchmark Comparison" ch={
                  <div>
                    ...
                    <div style={{display:"flex",padding:"10px 0",borderBottom:"1px solid "+G.border,background:"#e8ecf9"}}>
